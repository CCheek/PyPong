...
def mainloop():
    while constants.loop:
        if constants.right_paddle_up:
            init.right_paddle.move_up(constants.step)
        elif constants.right_paddle_down:
            init.right_paddle.move_down(constants.step)
        if constants.left_paddle_up:
            init.left_paddle.move_up(constants.step)
        elif constants.left_paddle_down:
            init.left_paddle.move_down(constants.step)
        ...
        for event in pygame.event.get():  # For all events (click, keyboard, controller, etc..)
            if event.type == pygame.QUIT:  # If user clicked close
                ...
                if event.key == pygame.K_UP:
                    ...
                    constants.left_paddle_up = False
                elif event.key == pygame.K_s:
                    constants.left_paddle_down = False
